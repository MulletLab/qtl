...
void prob_ft(double rf, int t, double *transpr)
{
  ...
  r2 = rf * rf;
  rw = w * rf;
  H = 0.8;   /* Change this hardcoded heterozygosity value */
  H1 = ((1 - H) / 2);
  ...
  transpr[0] = (H1 * H1) * SDt * w2 + SEt * r2 + sbetaBA;           /* PfA1 = PfDA + PfDEA + PfDBA */
  transpr[5] = transpr[0];                              /* PfA0 = PfA1 */

  transpr[2] = (H1 * H1) * SDt * r2 + SEt * w2 + sbetaBA;           /* PfbC = PfDC + PfDEC + PfDBC */
  transpr[3] = (H * H) * (beta1 + gamma1) / 2.0;                  /* PfD = PfDD */
  transpr[4] = (H * H) * (beta1 - gamma1) / 2.0;                  /* PfE = PfDE */
